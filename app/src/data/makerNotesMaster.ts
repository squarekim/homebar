/**
 * makerNotesMaster.ts — 기준 DB(liquor_master) 제품별 제조사/공식 테이스팅 노트.
 * 키는 마스터 제품 id 다. 누가 무엇을 보유했는지와 무관한 제품 정보라 공개 빌드에도 그대로 실린다
 * (개인 보유 병 id 로 키가 잡힌 makerNotes.ts 는 공개 빌드에서 스텁으로 치환된다).
 *
 * 규칙
 *  - 모든 노트는 source(URL)를 반드시 갖는다. 출처를 확인하지 못한 제품은 여기 없고,
 *    화면에는 "공식 노트 미확보"로 표시된다. 추정해서 채우지 않는다.
 *  - sourceName 은 그 출처가 제조사 공식인지, 매체·리테일러인지, 보도자료인지를 그대로 밝힌다.
 *
 * 쓰이는 곳: 술 추가 화면(제품 확인 단계)과 술장 카드 — data/userBottles.ts 가 masterId 로 붙인다.
 */
import { type MakerNote } from '../models/types';

export const MASTER_MAKER_NOTES: Record<string, MakerNote> = {
  m_olesmoky_pb: {
  ...
  m_jw_ruby: {
    nose: "잘 익은 라즈베리 잼과 블랙베리 파이, 깊은 꿀의 단맛",
    palate: "무화과·자두·마라스키노 체리의 진한 과일감에 옅은 스파이스",
    finish: "가벼운 스모크가 뒤에 남는다",
    text: "PX·올로로소 셰리와 레드 와인 캐스크 숙성분에 버번 캐스크를 더해 블랙 라벨보다 달게 잡은 블렌드(마스터 블렌더 엠마 워커).",
    source: "https://www.thespiritsbusiness.com/2025/03/johnnie-walker-black-ruby-launches-worldwide/",
  ...
  m_balvenie12dw: {
    ...
    source: "https://www.thebalvenie.com/en-us/our-whisky-collection/cask-finishes/doublewood-12/",
    sourceName: "The Balvenie 공식 — 숙성 설명(향미 3분할 노트는 공식 페이지에 없음)",
  },
};
