...
- **술 직접 추가** — 위스키 탭의 `+ 술 추가` 버튼으로 보유 술을 등록한다. 위스키면 원산지·타입·지역·캐스크·캐릭터까지 골라 저장하며, 저장 즉시 컬렉션·분류 배지·축별 커버리지·지역×숙성 매트릭스·구매 시뮬레이터에 반영된다(IndexedDB 영속, JSON 백업 포함, 사용자 추가분은 삭제 가능)
